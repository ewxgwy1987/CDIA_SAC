...
{
    public partial class SCPLCEM2GW : ServiceBase
    {
       private const string SERVICE_NAME = "PALS.SCPLCEM2GW";
        ...
        public SCPLCEM2GW()
        {
            ...
            if (!System.Diagnostics.EventLog.SourceExists("PALS.SCPLCEM2GW"))
            {
                System.Diagnostics.EventLog.CreateEventSource(
                    "PALS.SCPLCEM2GW", "Application");
            }
            eventLog1.Source = "PALS.SCPLCEM2GW";
        ...
        protected override void OnStart(string[] args)
        {
            ...
            try
            {
                ...
                string xmlSettingFile = PALS.Utilities.Functions.GetXMLFileFullName("PALS_BASE", "CFG_SCPLCEM2GW.xml", 5);
                if (xmlSettingFile == null)
                {
                    // Read XML configuration file from \MessageRouter sub folder.
                    xmlSettingFile = PALS.Utilities.Functions.GetXMLFileFullName("PALS_BASE", @"SAC\CFG_SCPLCEM2GW.xml", 5);

                    if (xmlSettingFile == null)
                        throw new Exception("XML configuration file (CFG_SCPLCEM2GW.xml) could not be found! " +
                                    "Please verify whether the Windows environment parameter (PALS_BASE) has been defined, " +
                                    "or file CFG_SCPLCEM2GW.xml is existing in the folder {PALS_BASE}\\SAC\\");
                }
