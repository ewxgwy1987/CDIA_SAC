...
namespace PALS.PLCSM5GW
{
    public partial class PLCSM5GW : ServiceBase
    {
       private const string SERVICE_NAME = "PALS.PLCSM5GW";
        ...
        public PLCSM5GW()
        {
            ...
            if (!System.Diagnostics.EventLog.SourceExists("PALS.PLCSM5GW"))
            {
                System.Diagnostics.EventLog.CreateEventSource(
                    "PALS.PLCSM5GW", "Application");
            }
            eventLog1.Source = "PALS.PLCSM5GW";
        ...
        protected override void OnStart(string[] args)
        {
            ...
            try
            {
                ...
                string xmlSettingFile = PALS.Utilities.Functions.GetXMLFileFullName("PALS_BASE", "CFG_PLCSM5GW.xml", 5);
                if (xmlSettingFile == null)
                {
                    // Read XML configuration file from \MessageRouter sub folder.
                    xmlSettingFile = PALS.Utilities.Functions.GetXMLFileFullName("PALS_BASE", @"SAC\CFG_PLCSM5GW.xml", 5);

                    if (xmlSettingFile == null)
                        throw new Exception("XML configuration file (CFG_PLCSM5GW.xml) could not be found! " +
                                    "Please verify whether the Windows environment parameter (PALS_BASE) has been defined, " +
                                    "or file CFG_PLCSM5GW.xml is existing in the folder {PALS_BASE}\\SAC\\");
                }
