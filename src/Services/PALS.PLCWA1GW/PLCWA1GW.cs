...
{
    public partial class PLCWA1GW : ServiceBase
    {
       private const string SERVICE_NAME = "PALS.PLCWA1GW";
        ...
        public PLCWA1GW()
        {
            ...
            if (!System.Diagnostics.EventLog.SourceExists("PALS.PLCWA1GW"))
            {
                System.Diagnostics.EventLog.CreateEventSource(
                    "PALS.PLCWA1GW", "Application");
            }
            eventLog1.Source = "PALS.PLCWA1GW";
        ...
        protected override void OnStart(string[] args)
        {
            ...
            try
            {
                ...
                string xmlSettingFile = PALS.Utilities.Functions.GetXMLFileFullName("PALS_BASE", "CFG_PLCWA1GW.xml", 5);
                if (xmlSettingFile == null)
                {
                    // Read XML configuration file from \MessageRouter sub folder.
                    xmlSettingFile = PALS.Utilities.Functions.GetXMLFileFullName("PALS_BASE", @"SAC\CFG_PLCWA1GW.xml", 5);

                    if (xmlSettingFile == null)
                        throw new Exception("XML configuration file (CFG_PLCWA1GW.xml) could not be found! " +
                                    "Please verify whether the Windows environment parameter (PALS_BASE) has been defined, " +
                                    "or file CFG_PLCWA1GW.xml is existing in the folder {PALS_BASE}\\SAC\\");
                }
