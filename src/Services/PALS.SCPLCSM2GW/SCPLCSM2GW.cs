...
namespace PALS.SCPLCSM2GW
{
    public partial class SCPLCSM2GW : ServiceBase
    {
       private const string SERVICE_NAME = "PALS.SCPLCSM2GW";
        ...
        public SCPLCSM2GW()
        {
            ...
            if (!System.Diagnostics.EventLog.SourceExists("PALS.SCPLCSM2GW"))
            {
                System.Diagnostics.EventLog.CreateEventSource(
                    "PALS.SCPLCSM2GW", "Application");
            }
            eventLog1.Source = "PALS.SCPLCSM2GW";
        ...
        protected override void OnStart(string[] args)
        {
            ...
            try
            {
                ...
                string xmlSettingFile = PALS.Utilities.Functions.GetXMLFileFullName("PALS_BASE", "CFG_SCPLCSM2GW.xml", 5);
                if (xmlSettingFile == null)
                {
                    // Read XML configuration file from \MessageRouter sub folder.
                    xmlSettingFile = PALS.Utilities.Functions.GetXMLFileFullName("PALS_BASE", @"SAC\CFG_SCPLCSM2GW.xml", 5);

                    if (xmlSettingFile == null)
                        throw new Exception("XML configuration file (CFG_SCPLCSM2GW.xml) could not be found! " +
                                    "Please verify whether the Windows environment parameter (PALS_BASE) has been defined, " +
                                    "or file CFG_SCPLCSM2GW.xml is existing in the folder {PALS_BASE}\\SAC\\");
                }
