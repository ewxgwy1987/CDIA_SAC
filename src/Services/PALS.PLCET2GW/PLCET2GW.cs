...
{
    public partial class PLCET2GW : ServiceBase
    {
       private const string SERVICE_NAME = "PALS.PLCET2GW";
        ...
        public PLCET2GW()
        {
            ...
            if (!System.Diagnostics.EventLog.SourceExists("PALS.PLCET2GW"))
            {
                System.Diagnostics.EventLog.CreateEventSource(
                    "PALS.PLCET2GW", "Application");
            }
            eventLog1.Source = "PALS.PLCET2GW";
        ...
        protected override void OnStart(string[] args)
        {
            ...
            try
            {
                ...
                string xmlSettingFile = PALS.Utilities.Functions.GetXMLFileFullName("PALS_BASE", "CFG_PLCET2GW.xml", 5);
                if (xmlSettingFile == null)
                {
                    // Read XML configuration file from \MessageRouter sub folder.
                    xmlSettingFile = PALS.Utilities.Functions.GetXMLFileFullName("PALS_BASE", @"SAC\CFG_PLCET2GW.xml", 5);

                    if (xmlSettingFile == null)
                        throw new Exception("XML configuration file (CFG_PLCET2GW.xml) could not be found! " +
                                    "Please verify whether the Windows environment parameter (PALS_BASE) has been defined, " +
                                    "or file CFG_PLCET2GW.xml is existing in the folder {PALS_BASE}\\SAC\\");
                }
