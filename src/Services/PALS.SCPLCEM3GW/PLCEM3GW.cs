...
namespace PALS.PLCEM3GW
{
    public partial class PLCEM3GW : ServiceBase
    {
       private const string SERVICE_NAME = "PALS.PLCEM3GW";
        ...
        public PLCEM3GW()
        {
            ...
            if (!System.Diagnostics.EventLog.SourceExists("PALS.PLCEM3GW"))
            {
                System.Diagnostics.EventLog.CreateEventSource(
                    "PALS.PLCEM3GW", "Application");
            }
            eventLog1.Source = "PALS.PLCEM3GW";
        ...
        protected override void OnStart(string[] args)
        {
            ...
            try
            {
                ...
                string xmlSettingFile = PALS.Utilities.Functions.GetXMLFileFullName("PALS_BASE", "CFG_PLCEM3GW.xml", 5);
                if (xmlSettingFile == null)
                {
                    // Read XML configuration file from \MessageRouter sub folder.
                    xmlSettingFile = PALS.Utilities.Functions.GetXMLFileFullName("PALS_BASE", @"SAC\CFG_PLCEM3GW.xml", 5);

                    if (xmlSettingFile == null)
                        throw new Exception("XML configuration file (CFG_PLCEM3GW.xml) could not be found! " +
                                    "Please verify whether the Windows environment parameter (PALS_BASE) has been defined, " +
                                    "or file CFG_PLCEM3GW.xml is existing in the folder {PALS_BASE}\\SAC\\");
                }
