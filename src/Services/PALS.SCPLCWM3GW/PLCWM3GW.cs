...
{
    public partial class PLCWM3GW : ServiceBase
    {
       private const string SERVICE_NAME = "PALS.PLCWM3GW";
        ...
        public PLCWM3GW()
        {
            ...
            if (!System.Diagnostics.EventLog.SourceExists("PALS.PLCWM3GW"))
            {
                System.Diagnostics.EventLog.CreateEventSource(
                    "PALS.PLCWM3GW", "Application");
            }
            eventLog1.Source = "PALS.PLCWM3GW";
        ...
        protected override void OnStart(string[] args)
        {
            ...
            try
            {
                ...
                string xmlSettingFile = PALS.Utilities.Functions.GetXMLFileFullName("PALS_BASE", "CFG_PLCWM3GW.xml", 5);
                if (xmlSettingFile == null)
                {
                    // Read XML configuration file from \MessageRouter sub folder.
                    xmlSettingFile = PALS.Utilities.Functions.GetXMLFileFullName("PALS_BASE", @"SAC\CFG_PLCWM3GW.xml", 5);

                    if (xmlSettingFile == null)
                        throw new Exception("XML configuration file (CFG_PLCWM3GW.xml) could not be found! " +
                                    "Please verify whether the Windows environment parameter (PALS_BASE) has been defined, " +
                                    "or file CFG_PLCWM3GW.xml is existing in the folder {PALS_BASE}\\SAC\\");
                }
