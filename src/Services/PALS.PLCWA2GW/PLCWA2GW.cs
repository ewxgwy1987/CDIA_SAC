...
namespace PALS.PLCWA2GW
{
    public partial class PLCWA2GW : ServiceBase
    {
       private const string SERVICE_NAME = "PALS.PLCWA2GW";
        ...
        public PLCWA2GW()
        {
            ...
            if (!System.Diagnostics.EventLog.SourceExists("PALS.PLCWA2GW"))
            {
                System.Diagnostics.EventLog.CreateEventSource(
                    "PALS.PLCWA2GW", "Application");
            }
            eventLog1.Source = "PALS.PLCWA2GW";
        ...
        protected override void OnStart(string[] args)
        {
            ...
            try
            {
                ...
                string xmlSettingFile = PALS.Utilities.Functions.GetXMLFileFullName("PALS_BASE", "CFG_PLCWA2GW.xml", 5);
                if (xmlSettingFile == null)
                {
                    // Read XML configuration file from \MessageRouter sub folder.
                    xmlSettingFile = PALS.Utilities.Functions.GetXMLFileFullName("PALS_BASE", @"SAC\CFG_PLCWA2GW.xml", 5);

                    if (xmlSettingFile == null)
                        throw new Exception("XML configuration file (CFG_PLCWA2GW.xml) could not be found! " +
                                    "Please verify whether the Windows environment parameter (PALS_BASE) has been defined, " +
                                    "or file CFG_PLCWA2GW.xml is existing in the folder {PALS_BASE}\\SAC\\");
                }
