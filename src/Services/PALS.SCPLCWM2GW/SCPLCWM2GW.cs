...
{
    public partial class SCPLCWM2GW : ServiceBase
    {
       private const string SERVICE_NAME = "PALS.SCPLCWM2GW";
        ...
        public SCPLCWM2GW()
        {
            ...
            if (!System.Diagnostics.EventLog.SourceExists("PALS.SCPLCWM2GW"))
            {
                System.Diagnostics.EventLog.CreateEventSource(
                    "PALS.SCPLCWM2GW", "Application");
            }
            eventLog1.Source = "PALS.SCPLCWM2GW";
        ...
        protected override void OnStart(string[] args)
        {
            ...
            try
            {
                ...
                string xmlSettingFile = PALS.Utilities.Functions.GetXMLFileFullName("PALS_BASE", "CFG_SCPLCWM2GW.xml", 5);
                if (xmlSettingFile == null)
                {
                    // Read XML configuration file from \MessageRouter sub folder.
                    xmlSettingFile = PALS.Utilities.Functions.GetXMLFileFullName("PALS_BASE", @"SAC\CFG_SCPLCWM2GW.xml", 5);

                    if (xmlSettingFile == null)
                        throw new Exception("XML configuration file (CFG_SCPLCWM2GW.xml) could not be found! " +
                                    "Please verify whether the Windows environment parameter (PALS_BASE) has been defined, " +
                                    "or file CFG_SCPLCWM2GW.xml is existing in the folder {PALS_BASE}\\SAC\\");
                }
