...
{
    public partial class PLCWT1GW : ServiceBase
    {
       private const string SERVICE_NAME = "PALS.PLCWT1GW";
        ...
        public PLCWT1GW()
        {
            ...
            if (!System.Diagnostics.EventLog.SourceExists("PALS.PLCWT1GW"))
            {
                System.Diagnostics.EventLog.CreateEventSource(
                    "PALS.PLCWT1GW", "Application");
            }
            eventLog1.Source = "PALS.PLCWT1GW";
        ...
        protected override void OnStart(string[] args)
        {
            ...
            try
            {
                ...
                string xmlSettingFile = PALS.Utilities.Functions.GetXMLFileFullName("PALS_BASE", "CFG_PLCWT1GW.xml", 5);
                if (xmlSettingFile == null)
                {
                    // Read XML configuration file from \MessageRouter sub folder.
                    xmlSettingFile = PALS.Utilities.Functions.GetXMLFileFullName("PALS_BASE", @"SAC\CFG_PLCWT1GW.xml", 5);

                    if (xmlSettingFile == null)
                        throw new Exception("XML configuration file (CFG_PLCWT1GW.xml) could not be found! " +
                                    "Please verify whether the Windows environment parameter (PALS_BASE) has been defined, " +
                                    "or file CFG_PLCWT1GW.xml is existing in the folder {PALS_BASE}\\SAC\\");
                }
