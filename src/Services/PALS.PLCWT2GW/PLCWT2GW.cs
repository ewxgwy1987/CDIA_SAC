...
{
    public partial class PLCWT2GW : ServiceBase
    {
       private const string SERVICE_NAME = "PALS.PLCWT2GW";
        ...
        public PLCWT2GW()
        {
            ...
            if (!System.Diagnostics.EventLog.SourceExists("PALS.PLCWT2GW"))
            {
                System.Diagnostics.EventLog.CreateEventSource(
                    "PALS.PLCWT2GW", "Application");
            }
            eventLog1.Source = "PALS.PLCWT2GW";
        ...
        protected override void OnStart(string[] args)
        {
            ...
            try
            {
                ...
                string xmlSettingFile = PALS.Utilities.Functions.GetXMLFileFullName("PALS_BASE", "CFG_PLCWT2GW.xml", 5);
                if (xmlSettingFile == null)
                {
                    // Read XML configuration file from \MessageRouter sub folder.
                    xmlSettingFile = PALS.Utilities.Functions.GetXMLFileFullName("PALS_BASE", @"SAC\CFG_PLCWT2GW.xml", 5);

                    if (xmlSettingFile == null)
                        throw new Exception("XML configuration file (CFG_PLCWT2GW.xml) could not be found! " +
                                    "Please verify whether the Windows environment parameter (PALS_BASE) has been defined, " +
                                    "or file CFG_PLCWT2GW.xml is existing in the folder {PALS_BASE}\\SAC\\");
                }
