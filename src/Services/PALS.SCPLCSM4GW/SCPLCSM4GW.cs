...
{
    public partial class SCPLCSM4GW : ServiceBase
    {
       private const string SERVICE_NAME = "PALS.SCPLCSM4GW";
        ...
        public SCPLCSM4GW()
        {
            ...
            if (!System.Diagnostics.EventLog.SourceExists("PALS.SCPLCSM4GW"))
            {
                System.Diagnostics.EventLog.CreateEventSource(
                    "PALS.SCPLCSM4GW", "Application");
            }
            eventLog1.Source = "PALS.SCPLCSM4GW";
        ...
        protected override void OnStart(string[] args)
        {
            ...
            try
            {
                ...
                string xmlSettingFile = PALS.Utilities.Functions.GetXMLFileFullName("PALS_BASE", "CFG_SCPLCSM4GW.xml", 5);
                if (xmlSettingFile == null)
                {
                    // Read XML configuration file from \MessageRouter sub folder.
                    xmlSettingFile = PALS.Utilities.Functions.GetXMLFileFullName("PALS_BASE", @"SAC\CFG_SCPLCSM4GW.xml", 5);

                    if (xmlSettingFile == null)
                        throw new Exception("XML configuration file (CFG_SCPLCSM4GW.xml) could not be found! " +
                                    "Please verify whether the Windows environment parameter (PALS_BASE) has been defined, " +
                                    "or file CFG_SCPLCSM4GW.xml is existing in the folder {PALS_BASE}\\SAC\\");
                }
